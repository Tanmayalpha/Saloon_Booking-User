...
import 'package:beauty_salons_customer/common/map.dart';
...
class SalonView extends GetView<SalonController> {
      ...
      if (!_salon.hasData) {
        ...
        return Scaffold(
         /* floatingActionButton: controller.currentIndex.value == 1
              ...
              : null,*/
          bottomNavigationBar: controller.currentIndex.value == 0 ? buildBottomWidget() : null,
          body: !controller.loading.value?RefreshIndicator(
              onRefresh: () async {
                  ...
                  SliverAppBar(
                      ...
                      Obx(() {
                        if (controller.salon.value.address == null)
                          ...
                          return MaterialButton(
                            elevation: 0,
                            onPressed: () => MapsUtil.openMapsSheet(context, controller.salon.value.address?.getLatLng(), controller.salon.value.id),
                    ...
                    bottom: buildSalonTitleBarWidget(_salon,(){
                      if(Get.find<AuthService>().isAuth){
                        controller.addFav(_salon.isFavorites, _salon.id.toString());
                      }else{
  ...
  SalonTitleBarWidget buildSalonTitleBarWidget(Salon _salon,VoidCallback onPressed) {
          ...
          Row(
            children: [
              ...
              IconButton(onPressed: (){
                controller.addFav(_salon.isFavorites, _salon.id);
              }, icon: Icon(_salon.isFavorites?Icons.favorite:Icons.favorite_border,color: _salon.isFavorites?Colors.red:Colors.grey,)),
              SizedBox(width: 5),
              /*Container(
          ...
          Expanded(
            child: BlockButtonWidget(
                ...
                onPressed: _booking.eServices.isEmpty
                    ? null
                    : () {
                        print(_booking.eServices.length);
                        _booking.salon = Get.find<SalonController>().salon.value;
                        print(_booking.eServices.length);
                        Get.toNamed(Routes.BOOK_E_SERVICE, arguments: {'booking': _booking,'is_offer':controller.fromOffer?"1":"0"});
                      }),
          ),
          SizedBox(width: 20),
          Wrap(
