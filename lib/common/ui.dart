...
class Ui {
  ...
  /// new otp Snack Bar
  static GetSnackBar SetSnackBar({String title = 'Otp', String message, SnackPosition snackPosition = SnackPosition.BOTTOM}) {
    Get.log("[$title] $message", isError: true);
    return GetSnackBar(
      titleText: Text(title.tr, style: Get.textTheme.headline6.merge(TextStyle(color: Get.theme.primaryColor))),
      messageText: Text(message.substring(0, min(message.length, 150)), style: Get.textTheme.caption.merge(TextStyle(color: Get.theme.primaryColor))),
      snackPosition: snackPosition,
      margin: EdgeInsets.all(20),
  ...
  static String filterData(Salon salon){
    if(salon.availabilityHours.length>0){
      String first = salon.availabilityHours.first.day.toString().substring(0,3);
      String last = salon.availabilityHours.last.day.toString().substring(0,3);
      return "${first}-${last}  |  ${DateFormatString(salon.availabilityHours.first.startAt)}-${DateFormatString(salon.availabilityHours.first.endAt)}";
    }
    return "";
  }
